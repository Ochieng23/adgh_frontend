      ...
      await transporter.sendMail({
        ...
        html: `
          ...
          <table style="border-collapse:collapse;width:100%;max-width:600px;font-family:system-ui,sans-serif;font-size:14px;">
            <tr><td style="padding:8px 0;color:#7A8C7F;width:130px;">Name</td><td style="padding:8px 0;color:#3A4A3E;">${name}</td></tr>
            <tr><td style="padding:8px 0;color:#7A8C7F;">Email</td><td style="padding:8px 0;"><a href="mailto:${email}" style="color:#C9973A;">${email}</a></td></tr>
            <tr><td style="padding:8px 0;color:#7A8C7F;">Organisation</td><td style="padding:8px 0;color:#3A4A3E;">${organisation || '—'}</td></tr>
            <tr><td style="padding:8px 0;color:#7A8C7F;">Subject</td><td style="padding:8px 0;color:#3A4A3E;">${subject}</td></tr>
          </table>
          <div style="margin-top:16px;padding:16px;background:#FAF7F2;border-left:4px solid #C9973A;border-radius:4px;">
            <p style="margin:0;color:#3A4A3E;font-size:14px;line-height:1.7;">${message.replace(/\n/g, '<br>')}</p>
          </div>
        `,
      })
    } else {
